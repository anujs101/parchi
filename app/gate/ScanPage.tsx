...
export default function ScanPage() {
  ...
  async function startWalletConnectAndSign(messageToSignStr: string) {
    ...
    try {
      ...
      const session = await Promise.race([approvalPromise, timeoutPromise]) as SessionTypes.Struct;
      sessionTopicRef.current = session.topic;
      setWcStatus('connected');

      // prepare request for Phantom (UTF-8 text)
      const request = {
        topic: session.topic,
        chainId: 'solana:mainnet',
        request: {
          method: 'solana_signMessage',
          params: { message: messageToSignStr },
        },
      };
